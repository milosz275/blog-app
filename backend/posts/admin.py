...
class BlogPostInline(admin.StackedInline):
    model = BlogPost
    fields = ["title", "description", "content", "image_file", "image_tag", "download_image"]
    extra = 0
    ...
    def image_tag(self, obj):
        if obj.image_data:
            return mark_safe(f'<img src="data:image/png;base64,{obj.image_data}" style="max-height: 100px;"/>')
        return "No image"
    ...
    def get_urls(self):
        ...
        urls = super().get_urls()
        custom_urls = [
            path("download-image/<int:pk>/", self.admin_site.admin_view(self.download_image_view), name="download_image"),
        ]
    ...
    def download_image_view(self, request, pk):
        ...
        if asset_image and asset_image.image_data:
            ...
            response["Content-Disposition"] = f'attachment; filename="course_asset_image_{pk}.png"'
            return response
...
class BlogPostAdmin(admin.ModelAdmin):
    ...
    def image_tag(self, obj):
        if obj.image_data:
            return mark_safe(f'<img src="data:image/png;base64,{obj.image_data}" style="max-height: 200px;"/>')
        return "No image"
    ...
    fieldsets = (
        (None, {
            "fields": ("user", "title", "description", "content", "image_file", "image_tag", "download_image")
        }),
        ("Timestamps", {
    ...
    def download_image_view(self, request, pk):
        ...
        if user_profile and user_profile.image_data:
            ...
            response = HttpResponse(image_data, content_type="image/png")
            response["Content-Disposition"] = f'attachment; filename="post_image_{pk}.png"'
            return response
